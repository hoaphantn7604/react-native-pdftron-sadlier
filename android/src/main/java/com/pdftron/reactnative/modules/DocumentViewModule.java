...
public class DocumentViewModule extends ReactContextBaseJavaModule implements ActivityEventListener {
    ...
    @ReactMethod
    public void importBookmarks(final int tag, final String boomark,  final Promise promise) {
        getReactApplicationContext().runOnUiQueueThread(new Runnable() {
            @Override
            public void run() {
                try {
                   mDocumentViewInstance.importBookmark(tag, boomark);
                    promise.resolve(null);
                } catch (Exception ex) {
                    promise.reject(ex);
                }
    ...
    @ReactMethod
    public void pageLabel(final int tag, final int item) {
        getReactApplicationContext().runOnUiQueueThread(new Runnable() {
            @Override
            public void run() {
                try {
